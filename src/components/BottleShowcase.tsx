...
// Daftarkan plugin ScrollTrigger satu kali
...
export default function BottleShowcase() {
  ...
  useEffect(() => {
    // Periksa preferensi prefers-reduced-motion (mengurangi animasi)
    ...
    const ctx = gsap.context(() => {
      if (prefersReducedMotion) {
        // Tata letak fallback untuk prefers-reduced-motion (tanpa pin scroll, hanya fade-in sederhana)
        gsap.fromTo(
      ...
      // 1. Buat timeline scroll scrub yang dipin ke viewport
      ...
      // 2. Animasikan elemen saat pengguna melakukan scroll
      // Judul latar belakang besar memudar masuk (fade in), membesar, dan naik perlahan
      tl.fromTo(
        ".showcase-bg-heading",
      ...
      // Botol membesar dari kecil ke besar, sedikit berputar, dan bergerak ke atas
      tl.fromTo(
        ".showcase-bottle",
      ...
      // Callout 1 (Obat Original, sisi kiri) memudar masuk dan bergeser ke posisi kiri
      tl.fromTo(
        ".callout-1",
      ...
      // Efek fade-out lembut pada callout-1 saat scroll semakin dalam untuk memberi ruang pada fitur berikutnya
      tl.to(
        ".callout-1",
      ...
      // Callout 2 (Stok Live, sisi kanan) memudar masuk dan bergeser ke kanan
      tl.fromTo(
        ".callout-2",
        { opacity: 0, x: isMobile ? 30 : 100, scale: 0.9 },
        { opacity: 1, x: 0, scale: 1, duration: 1 },
      ...
      // Callout 3 (Konsultasi Langsung, di bawah-tengah atau kiri) bergeser ke atas
      tl.fromTo(
        ".callout-3",
  ...
  return (
    ...
    >
      {/* Spot gradien dekoratif di latar belakang */}
      ...
      {/* Tampilan viewport layar penuh utama yang dipin */}
      <div
        ref={containerRef}
        className="w-full min-h-screen flex flex-col justify-center items-center px-4 relative"
      >
        {/* Teks judul watermark besar di belakang botol */}
        ...
        <div className="w-full max-w-5xl mx-auto h-[500px] md:h-[600px] relative flex items-center justify-center showcase-content">
          ...
          {/* Kartu callout fitur yang melayang */}
          {/* Fitur 1 - Kiri */}
          ...
          {/* Fitur 3 - Tengah-Bawah atau Kiri-Bawah (responsif) */}
        ...
        {/* Indikator scroll interaktif mini di bagian bawah tata letak */}
